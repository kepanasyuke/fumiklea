...
def generate_scenes():
    ...
    for scene_idx in range(TOTAL_SCENES):
        ...
        for f_idx in range(TOTAL_FRAMES):
         ...
         # --- СЦЕНА 6: Взрыв, плавление, белый пар и залив экрана брызгами ---
            elif scene_idx == 6:
                # Строки 24-32 строго черные, бережём индикаторы (РЕМ, РАС, ТОП) внизу!
                
                # 1. МАЛЕНЬКАЯ ГОЛУБАЯ ЛУЖА И ЧЁРНЫЙ ПРОМЕЖУТОК
                # Делаем лужу аккуратной: тонкая голубая линия на строке 22, шириной от 6 до 26 пикселей
                frame[22, 6:26] = C['BLU']
                ...
                if f_idx >= 28:
                    ...
                    # Машина едет вперед сквозь черный промежуток к луже
                    cy = int(8 + car_progress * 9) 
                    
                    if 0 <= cy < 23:
                        # Рисуем Кристину (если экран еще не залит брызгами в конце)
                        if f_idx < 44:
                            frame[cy:cy+2, 4:28] = C['RED']       # Капот
                            frame[cy, 11:21] = C['B_RED']         # Выштамповка
                            if cy+2 < 22: frame[cy+2, 8:24] = C['BLK']
                            if cy+3 < 22: frame[cy+3, 8:24] = C['CHRM']  # Решетка
                            if cy+2 < 22:
                                frame[cy+1:cy+3, 5:8] = C['WHT']   # Фары
                                frame[cy+1:cy+3, 24:27] = C['WHT']
                            if cy+4 < 22: frame[cy+4, 4:28] = C['CHRM']  # Бампер

                        # ЭФФЕКТ ТЕКУЩЕЙ КРАСКИ (с 34 по 43 кадр, пока машина видна)
                        if 34 <= f_idx < 44:
                        ...
                        # --- КРИСТИНА ВЛЕТАЕТ В ЛУЖУ: БЕЛЫЙ ПАР И ЭФФЕКТ ЗАЛИВА ЭКРАНА ---
                        if cy + 4 >= 20:
                            # 1. СТИЛЬНЫЙ БЕЛЫЙ ПАР (Пошаговый алгоритм, как у выхлопов в Сцене 3)
                            # Создаем 3 стадии растущих и рассеивающихся белых клубов пара
                            steam_stage = (f_idx - 36) % 6
                            for px_pipe in [8, 23]: # Дым идет из зон перегретого мотора над лужей
 # Дым идет из зон перегретого мотора над лужей
                                # Стадия 1: Тонкая струя чисто-белого пара прямо над бампером
                                if steam_stage > 0 and cy - 1 >= 0:
                                    frame[cy - 1, px_pipe] = C['WHT']
                                # Стадия 2: Облако пара расширяется и поднимается выше к капоту
                                if steam_stage > 2 and cy - 2 >= 0:
                                    frame[cy - 2, (px_pipe - 1) : (px_pipe + 2)] = C['WHT']
                                # Стадия 3: Пар рассеивается в воздухе (шахматные белые пиксели)
                            ...
                            # 2. ФИНАЛЬНЫЙ ЭФФЕКТ: ВОЛНА БРЫЗГ ЗАЛИВАЕТ ЭКРАН! (Кадры 44-49)
                            # Машина полностью проезжает по воде, тушит пламя, и брызги летят прямо в камеру
                            if f_idx >= 44:
                                # Очищаем верхний огонь, показывая, что Кристина цела и огонь сбит
                                frame[0:15, :] = C['BLK']
                                
                                splash_progress = (f_idx - 44) # от 0 до 5
                                # Волна брызг (белые и бирюзовые пиксели) поднимается снизу вверх, заполняя экран
                                splash_y_start = max(0, 22 - splash_progress * 5)
                                
                                for sy in range(splash_y_start, 24):
                                    for sx in range(WIDTH):
                                        # Создаем красивую текстуру летящей пены и брызг воды
                                        if (sx + sy + f_idx) % 2 == 0:
                                            frame[sy, sx] = C['WHT']
                                        elif (sx - sy + f_idx) % 3 == 0:
                                            frame[sy, sx] = C['BLU']
