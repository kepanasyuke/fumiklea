...
def generate_scenes():
    ...
    for scene_idx in range(TOTAL_SCENES):
        ...
        for f_idx in range(TOTAL_FRAMES):
            ...
            # --- СЦЕНА 3: Задние плавники (Кристина уезжает в ночь) ---
            elif scene_idx == 3:
                # 1. Отрисовка культовой кормы Plymouth Fury 1958 года (Вид сзади)
                frame[16:20, 4:28] = C['RED']       # Основная часть багажника
                frame[19:21, 4:28] = C['CHRM']      # Массивный задний бампер
                frame[15, 8:24] = C['BLK']         # Заднее стекло

                # 2. Хромированные молдинги и знаменитые острые плавники по бокам
                frame[12:17, 4] = C['CHRM']         # Левый хромированный кант плавника
                frame[12:17, 27] = C['CHRM']        # Правый хромированный кант плавника
                
                # Заполнение самих плавников красным цветом кузова
                for y in range(12, 17):
                    frame[y, 5:7] = C['RED']
                    frame[y, 25:27] = C['RED']

                # 3. Яростные, пульсирующие габаритные огни (внутри плавников)
                # Имитируем зловещее мерцание стоп-сигналов уезжающей машины
                # Пульсация зависит от кадра f_idx
                pulse = int(140 + np.sin(f_idx * 0.5) * 115)
                TAIL_LIGHT_COLOR = [pulse, 0, 0]

                # Левые и правые нижние стопы (встроены под плавники)
                frame[16:18, 5:7] = TAIL_LIGHT_COLOR
                frame[16:18, 25:27] = TAIL_LIGHT_COLOR
                
                # Выхлопные газы из-под бампера (появляются циклично, Кристина газует)
                smoke_density = int((f_idx % 10) * 0.8)
                for sx in [6, 7, 24, 25]:  # Точки выхлопных труб слева и справа
                    for sy in range(1, 3):
                        if 21 + sy < HEIGHT:
                            # Серый дым смешивается с темнотой
                            frame[21 + sy, sx + (f_idx % 2)] = [80, 80, 90]
