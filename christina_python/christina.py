            ...
            # --- СЦЕНА 2: Спидометр (Взгляд из салона: Стрелка ложится за 120) ---
                ...
                for i in range(10):
                    ...
                    if 0 <= y < HEIGHT and 0 <= x < WIDTH:
                       ...
                       # --- СЦЕНА 3: Легендарный плавник (Кристина уезжает в ночь) ---
            elif scene_idx == 3:
                # 1. Ночная дорога и перспектива (Кристина уезжает в правый верхний угол)
                # Движущаяся разметка уходит по диагонали
                road_step = (f_idx * 2) % 16
                for y in range(16, HEIGHT):
                    line_x = int(8 + (y - 16) * 0.5 + road_step) % WIDTH
                    if 0 <= line_x < WIDTH:
                        frame[y, line_x] = [60, 60, 60]  # Серые пиксели дороги

                # 2. Отрисовка левого борта Кристины в перспективе (уходит от центра влево-вниз)
                # Борт сужается к переду машины (вдалеке)
                for x in range(2, 18):
                    # Верхняя кран кузова (линия окон)
                    top_y = int(14 + (x - 2) * 0.15)
                    # Нижний край кузова (у бампера)
                    bottom_y = int(18 + (x - 2) * 0.25)
                    if top_y < HEIGHT and bottom_y < HEIGHT:
                        frame[top_y:bottom_y, x] = C['RED']       # Красный металл борта
                        frame[bottom_y, x] = C['CHRM']            # Молдинг по низу порога

                # 3. КОРМА И ГИГАНТСКИЙ ЗАДНИЙ ПЛАВНИК (На переднем плане слева, x от 16 до 26)
                # Плавник идет по диагонали вверх-назад, образуя острый угол
                frame[12:22, 16:20] = C['RED']   # Основание заднего крыла
                frame[20:23, 15:26] = C['CHRM']  # Тяжелый хромированный задний бампер

                # Хромированная острая окантовка левого плавника (стремительная линия)
                # Она поднимается из глубины (y=14) на передний план (y=8)
                for i in range(10):
                    px = 16 + i
                    py = 14 - int(i * 0.6)  # Уходит вверх, создавая острие плавника
                    if 0 <= px < WIDTH and 0 <= py < HEIGHT:
                        frame[py, px] = C['CHRM']
                        # Под хромом — знаменитая клиновидная вставка плавника
                        frame[py+1:20, px] = C['RED']

                # 4. ЗЛОВЕЩИЙ КЛИНОВИДНЫЙ СТОП-СИГНАЛ (Встроен прямо в этот плавник)
                # Пульсирует от кадра к кадру, создавая ощущение, что машина тормозит или газует
                pulse = int(150 + np.sin(f_idx * 0.4) * 105)
                TAIL_LIGHT = [pulse, 0, 0]

                # Зажигаем характерную вытянутую фару внутри плавника
                frame[12:15, 20:23] = TAIL_LIGHT
                frame[13:16, 21] = TAIL_LIGHT

                # 5. Клубы выхлопного дыма (вылетают из-под бампера внизу справа)
                smoke_t = f_idx % 12
                for i in range(3):
                    sx = int(22 + smoke_t + i * 2)
                    sy = int(21 + np.sin(smoke_t * 0.5) * 2)
                    if 0 <= sx < WIDTH and 0 <= sy < HEIGHT:
                        frame[sy, sx] = [90, 90, 95]  # Сизый выхлопной дым V8
