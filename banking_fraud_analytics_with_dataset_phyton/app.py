...
# Руководство оператора комплаенса (по умолчанию свернуто)
...
# Надежная страховка от ошибки NameError
if 'filtered_df' in locals() or 'filtered_df' in globals():
    tool_df = filtered_df.copy()
else:
    tool_df = df.copy()

# Сборка понятных русских колонок на основе реального Kaggle-датасета из README
...
# ВЫПЛЫВАЮЩИЙ ИНСТРУМЕНТ: ПАНЕЛЬ ФИЛЬТРАЦИИ И ТАБЛИЦА СБОРОК ВНУТРИ ЕДИНОГО EXPANDER
with st.expander("Выплывающая панель точечной фильтрации и интерактивная таблица логов", expanded=False):
    ...
    # Первая строка фильтров: выпадающие списки (Категории)
    ...
    # Все три числовых слайдера выстроены вертикальным аккуратным столбиком друг под другом
    ...
    # УПРАВЛЕНИЕ ОТМЕТКАМИ ВНУТРИ ПОЛУЧЕННОГО СРЕЗА
    ...
    # Синхронизация фиксированной высоты управляющих кнопок
    st.markdown("""
        <style>
    ...
    # Отображаем интерактивную таблицу логов прямо внутри выплывающей панели
...
* **Сессии в логах:** Общее количество уникальных транзакционных запросов, поступивших от расчетных шлюзов в антифрод-систему за отчетный период. Тренд показывает динамику общей активности клиентов.
